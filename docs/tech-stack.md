...
修订：3.1；日期：2026-09-20；依据 [PRD 1.6](PRD.md)；选型不变，功能开发先于真实联调。
当前进入功能开发安排；本次规范修订不安装组件、不读真实聊天、不控制应用、不恢复旧模型考试。
...
不能依据版本号猜兼容。功能开发由主会话维护package.json和锁文件，不为文档搭空壳应用。
...
2026-09-18调查dotnet --list-sdks未列出SDK，执行时复核；安装SDK须另行安排，缺少SDK只影响相应构建/实测，不阻塞TS及原生源码开发。
...
先开发桌面、核心与适配功能，模拟集成及开发包可先交付，再真实接入联调、Windows内测及Mac适配。
浏览器扩展成品、自动更新、截图/OCR、任意应用控制、同项目多执行者不属于首批。

真实会话未选定/未测试，SDK和Mac条件待执行时复核；这些只限制对应联调，不再阻塞功能编码。
具体运行/控制权限、系统组件安装及签名费用尚需相应确认，不将现有109项测试当新产品验收。
